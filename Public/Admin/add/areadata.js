...
function appendCity(thiscon, Cityxz) {
    ...
    if ($(dataCityinput).data("value") != "") {
        ...
        var inputarryid = $(dataCityinput).data("id");
        if (inputarry.length > 0) {
            for (var i in inputarry) {
                $('.data-result').append("<span class=\"svae_box aui-titlespan\" data-code=\"" + inputarry[i] + "\" data-id=\"" + inputarryid + "\" data-name=\"" + inputarryname[i] + "\" onclick=\"removespan_area(this)\">" + inputarryname[i] + "<i>×<\/i><\/span>");
            }
        }
    }
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            for (var i in relations[$(con).data("code")]) {
                strVarCity += '<li><a href="javascript:;" class="d-item" data-code="' + relations[$(con).data("code")][i] + '" data-id=\"' + dataarrary[relations[$(con).data("code")][i]][2] + '\"  data-name=\"' + dataarrary[relations[$(con).data("code")][i]][0] + '\" onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[relations[$(con).data("code")][i]][0] + '<label>0</label></a></li>';
            }
            strVarCity += "<\/ul>";
            $('.data-container-city').html(strVarCity);
        } else {
            if (datatype == "duoxuan") {
                ...
                $('.data-result').append("<span class=\"svae_box aui-titlespan\" data-code=\"" + $(con).data("code") + "\" data-id=\"" + $(con).data("id") + "\" data-name=\"" + $(con).data("name") + "\" onclick=\"removespan_area(this)\">" + $(con).data("name") + "<i>×<\/i><\/span>");
...
function svae_City() {
    ...
    $(dataCityinput).attr("data-id", id);
    // $(dataCityinput).val(Cityname);
    var Cityname2 = $(dataCityinput).val();
    $(dataCityinput).attr('value',Cityname);
    if(Cityname2!=Cityname){
        $(dataCityinput).change(getAjaxFuwu(id));
    }
