var hyznstrVar = "";
    ...
    hyznstrVar += "                        <div class=\"aui_title\" style=\"cursor: move;\">选择服务小区<\/div>";
    ...
    hyznstrVar += "                          <div class=\"data-result aui-titlespan\"><em><\/em><\/div>";
    hyznstrVar += "                          <div class=\"data-tabs\">";
    hyznstrVar += "                            <ul>";
    hyznstrVar += "                              <li onclick=\"removenode(this,0)\" data-selector=\"tab-all\" class=\"active\"><a href=\"javascript:;\"><span>全部小区<\/span><em><\/em><\/a><\/li>";
    hyznstrVar += "                            <\/ul>";
    hyznstrVar += "                          <\/div>";
    hyznstrVar += "                          <!--职能选择-大类-->";
    ...
    hyznstrVar += "                                <div class=\"cate-list-title\"><em><\/em><\/div>";
...
for(var i in znhycode)
{
    ...
    //点击大类
								...
								var maxhy=znhycode;
									...
									{
									hyznstrVar += "<li><a href=\"javascript:;\" data-code=\""+znhycode[pid].maxhycode[j].CodeValue+"\"  data-id=\""+znhycode[pid].maxhycode[j].id+"\"  data-name=\""+znhycode[pid].maxhycode[j].CodeName+"\" class=\"d-cate\" onclick=\"show_third_node(this,"+pid+","+j+","+znhycode[pid].maxhycode[j].id+")\">"+ znhycode[pid].maxhycode[j].CodeName +"<\/a><\/li>";
									}		
    ...
    hyznstrVar += "                                <div class=\"tree-list jquery-localdata\">";
    hyznstrVar += "							<div style=\"height:360px\" class=\"item-table\">";
    ...
    hyznstrVar += "                                      <ul id=\"data-hyzn-d3j\" class=\"clearfix\">";
                     //点击大类
						  function show_third_node(conthis,pid,jid,id)
						  {
							  var id=$(conthis).data('id');
							  var phyid=$(conthis).data('code');
							  var phyname=$(conthis).data('name');
							  $(conthis).parents('.data-container').hide();
							  show_zn(pid+1,jid,phyname,id);
						  }
						  
						  
						  //展示职能
						  function show_zn(pid,phyid,phyname,id)
						  {
							  ...
							  for(var j in znhycode[pid].maxhycode)
							  {
								  ...
								  if(phyid==j){
							  hyznstrVar += "<li class=\"d-tree-active\"><a href=\"javascript:;\" data-code=\""+znhycode[pid].maxhycode[j].CodeValue+"\"  data-name=\""+znhycode[pid].maxhycode[j].CodeName+"\" class=\"d-tree\" onclick=\"show_zn_last("+pid+","+j+",'"+phyname+"','"+znhycode[pid].maxhycode[j].id+"',this)\">"+ ((znhycode[pid].maxhycode[j].CodeName.length>11)?znhycode[pid].maxhycode[j].CodeName.substring(0,11)+"..":znhycode[pid].maxhycode[j].CodeName) +"<label>0<\/label><\/a><\/li>";
								  show_zn_last(pid,j,phyname,id);
								  }else
								  {hyznstrVar += "<li class=\"d-tree\"><a href=\"javascript:;\" data-code=\""+znhycode[pid].maxhycode[j].CodeValue+"\"  data-name=\""+znhycode[pid].maxhycode[j].CodeName+"\" class=\"d-tree\" onclick=\"show_zn_last("+pid+","+j+",'"+phyname+"','"+znhycode[pid].maxhycode[j].id+"',this)\">"+ ((znhycode[pid].maxhycode[j].CodeName.length>11)?znhycode[pid].maxhycode[j].CodeName.substring(0,11):znhycode[pid].maxhycode[j].CodeName) +"<label>0<\/label><\/a><\/li>";
									  }
							  }
						  ...
						  function show_zn_last(pid,phyid,phyname,id,lithis)
						  {
							  abcs(pid,phyid,phyname,id,lithis);
							  }
						  function abcss(pid,phyid,phyname,id,lithis,data){
							   var hyznstrVar = "";
							  $(lithis).parent('li').siblings('li').attr("class","d-tree");
							  $(lithis).parent('li').attr("class","d-tree-active");
							  for(var k in data)
							  {
							  var thirdnode=data[k];
							   hyznstrVar += "<li><a title=\""+thirdnode.mingcheng+"\" href=\"javascript:;\" data-code=\""+thirdnode.id+"\" data-id=\""+thirdnode.id+"\" data-name=\""+thirdnode.mingcheng+"\" data-ditem=\""+thirdnode.id+"\" class=\"d-item\" onclick=\"ditem_select(this)\">"+thirdnode.mingcheng+"<\/a><\/li>";		
							  }
						  ...
						  if(datazntype=="duoxuan"){
							  ...
							  if($('.data-result span').length>100)
							  {
    ...
    hyznstrVar += "                            <\/div>";
    hyznstrVar += "                          <\/div>";
    hyznstrVar += "                          <div class=\"data-error\" style=\"display: none;\">最多只能选择 10 项<\/div>";
    hyznstrVar += "                        <\/div>";
    hyznstrVar += "                      <\/div><\/td>";
    hyznstrVar += "                  <\/tr>";
    hyznstrVar += "                  <tr>";
...
var datazntype="";
var xqtypes="";
function appendhyznbar(thiscon,znpe,types){
	datazninput=thiscon;
	datazntype=znpe;
	xqtypes=types;
...
function svae_znitem()
{
	...
	$(datazninput).attr('value',valname);
	$('#'+xqtypes).attr('value',val);
	$('.aui_state_box').remove();
