...
class UserController extends AuthController
{
	...
	public function list1Action()
	{
		...
		$sql = $m_user->field('id')->where($cond)->order('id desc')->page($page,$pagesize)->buildSql();
		$sql = $m_user->table($sql.' as t')->field('t.id')->buildSql();
		$order_list = $m_order
			->table('lb_user')
			->join("LEFT JOIN lb_order ON lb_order.openid = lb_user.openid")
			->field('lb_user.id,count(lb_order.openid) as order1')
			->where('lb_user.id in '.$sql)
		...
		$cond['_string'] = 'lb_user.status = 1';
		$user_list = $m_user->join("LEFT JOIN lb_master ON lb_master.id = lb_user.profession")
		...
		$this->assign('beixinxianshi_list', $beixinxianshi_list);
  ...
  //$upload->thumbRemoveOrigin = true; //上传图片后删除原图片  
   ...
   if (!$upload->upload()) {// 上传错误提示错误信息  
       ...
       $_POST['pic'] = $picname;
			...
			$m_user->location =  $_POST['shequtxt'];
// location = post['shequtxt']
			// 数据校验通过
			$m_user->add();
