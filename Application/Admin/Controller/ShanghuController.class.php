...
class ShanghuController extends AuthController
{
    ...
    public function renzhengAction()
    {
      ...
      $fuwu = $m_shanghu->join('lb_shanghufuwuxiangmu on lb_shanghu.id = lb_shanghufuwuxiangmu.shanghuid')
                        ->field('lb_shanghu.id,count(lb_shanghufuwuxiangmu.id) as fuwus')
                        ->where($cond)
                          ->page($page, $pagesize)
                          ->order('lb_shanghu.id desc')
                          ->group('lb_shanghu.id')
      ...
      foreach ($shanghu as $key => $shanghuid) 
    { 
      if(count($fuwu)){
        foreach ($fuwu as $k1 => $user) {
          if($shanghu[$key]['id'] == $user['id']){
            $shanghu[$key]['fuwus'] = $user['fuwus'];
          }else{
            if(!isset($shanghu[$key]['fuwus'])){
              $shanghu[$key]['fuwus'] = 0;
            }
          }
        }
...
// var_dump($shanghu);exit;
    ...
    public function tuijianAction()
    {
      ...
      // $cond['lb_shanghu.id'] = 'lb_shanghufuwuxiangmu.shanghuid';
