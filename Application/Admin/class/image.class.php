<?php
	...
	*/
	class Image {
		...
		*/
		function cut($name, $x, $y, $width, $height, $qz="s_"){
			$imgInfo=$this->getInfo($name);                 //获取图片信息
			/* 裁剪的位置不能超出背景图片范围 */
		if($width==$height){
			if($imgInfo['height']>$imgInfo['width']){
				  $width=round($imgInfo['width']);
				  $height=round($imgInfo['width']);
				  $x=0;
				  $y=(round($imgInfo['height'])-$height)/2;
				  }else{
				  $width=round($imgInfo['height']);
				  $height=round($imgInfo['height']);
				  $x=(round($imgInfo['width'])-$width)/2;
				  $y=0;
			    }
			}else{	
			  ...
			  if($imgInfo['height']>$imgInfo['width']){
				  $width=round($imgInfo['width']);
				  $height=round(($imgInfo['width']/1.5));
				  $x=0;
			  ...
			  {
				  $w=round($imgInfo['height'])*1.5;
				  $h=round($imgInfo['width'])/1.5;
				  if($imgInfo['width']>=$w){
				  $width=$w;
				  $height=round($imgInfo['height']); 
				  $x=($imgInfo['width']-$width)/2;
				  $y=0;
				  }else if($h<=$imgInfo['height'])
				  {
				  $width=round($imgInfo['width']);
				  $height=$h; 
				  $x=0;
				  $y=($imgInfo['height']-$h)/2;; 
				  }
		...
		private function kidOfImage($srcImg, $size, $imgInfo){
			$newImg = imagecreatetruecolor($size["width"], $size["height"]); //新的高度和宽度	
