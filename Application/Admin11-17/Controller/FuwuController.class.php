...
// echo $m_shanghu->getLastSQL();exit;
	...
	public function add2Action($user_id)
	{
		...
		if(IS_POST){
			...
			$url = $tupian[1]['name']  . ',' . $tupian[2]['name'] . ',' .$tupian[3]['name'];
			...
			$m_shanghufuwu->starttime = $_POST['starttime'];
			$m_shanghufuwu->status = $_POST['status'];
			$m_shanghufuwu->add();
		}
		...
		$leibie1 = $m_fuwuleibie
		  		->table('lb_fuwuleibie')
		  		->join('LEFT JOIN  lb_fuwuxiangmu ON lb_fuwuleibie.sid = lb_fuwuxiangmu.erjileibie')
		  		->field('lb_fuwuleibie.*,lb_fuwuxiangmu.erjileibie as rid')
		  		->where('lb_fuwuleibie.sid != 0')
		  		->select();
