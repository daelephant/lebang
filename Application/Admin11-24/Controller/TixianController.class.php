...
//////三表联合查询$Model = M('T1');$Model->join('t2 on t1.id = t2.uid', 'left')->join('t3 on t2.uid = t3.sid', 'left')->select();					
	...
	public function list1Action()
		...
		//////三表联合查询$Model = M('T1');$Model->join('t2 on t1.id = t2.uid', 'left')->join('t3 on t2.uid = t3.sid', 'left')->select();					
					...
					$m_tixian = M('Tixian');
					// 通用查询条件$map['_string'] = 'status=1 AND score>10';
					//$map['_string'] = 'lb_tixian.status=0 OR lb_tixian.status=2';
					$cond['lb_tixian.status']='0';
					//$cond['lb_tixian.status']='2';
					//$cond['_logic']='OR';//'_logic'=>'or' $condition['_logic'] = 'OR';
					...
					->select();
					 //echo $m_tixian->getLastSQL();
					 //SELECT lb_tixian.*,lb_user.name as name,lb_user.phone as phone,lb_user.idnumber as idnumber,lb_bank.bank as bank,lb_bank.banknum as banknum FROM `lb_tixian` LEFT JOIN lb_user ON lb_tixian.openid = lb_user.openid LEFT JOIN lb_bank ON lb_tixian.bankid = lb_bank.id where lb_tixian.status!=1 GROUP BY id ORDER BY id desc LIMIT 0,10
					//var_dump($tixian_list);exit;
		// 将日期格式化
		foreach ($tixian_list as &$tixian) 
		{	
	...
	{	
		...
		{	
		   ...
		   $m = $jine->where('openid="'.$openid.'"')->setInc('money',$moeny);
		   
		   $jine->where('openid="'.$openid.'"')->setInc('moneys',$moeny);
