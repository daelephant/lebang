...
class UserController extends Controller
{
	...
	public function list1Action()
	{
		...
		$cond['status'] = '1';
		$user_list = $m_user
			->field('id,nickname,phone,idnumber,status,status,regtime,logintimes')
			->where($cond)
			->order('id desc')
			->page($page, $pagesize)
			->group('id')
...
//end
	...
	{	
		...
		$where['regtime'] = array('between',[$_GET['starttime'],$_GET['endtime']]);
		...
		//$where['_logic']='OR';
			...
			$user_list = $m_user
			->field('id,name,nickname,phone,idnumber,status,status,regtime,logintimes')
			->where($where)
			->order('id desc')
			->page($page, $pagesize)
