...
class OrderController extends AuthController
{
	...
	public function listAction()
	{
		...
		$cond['status'] = '3';
		$order_list = $m_order
			->field('id,location,tel,ordernum,demand,addtime,status,endtime,money,starttime,jdtime,paytime,oname')
			->where($cond)
			->order('id desc')
			->page($page, $pagesize)
			->select();
	...
	public function editAction($order_id)
	{
		...
		//$order_list = $m_order//->join('RIGHT JOIN work ON artist.id = work.artist_id')->select()
			...
			//匹配jdopenid
					...
					->field('lb_order.*,lb_user.name and phone as username1')
					//->order('id desc')
					->find();
 ...
 */
	...
	{	
		...
		if ($_GET['sosuo'] == 1) {
			$where['status'] = 3;
			$where['tel']=$_GET['value'];
		}
		if ($_GET['sosuo'] == 2) {
			$where['ordernum']=$_GET['value'];
		}
		//$where['_logic']='OR';
...
/**
	...
	public function cancelAction()
	{
		...
		$cond['status'] = '5';
		$order_list = $m_order
			->field('id,location,tel,ordernum,demand,addtime,status,endtime,money,starttime,jdtime,paytime,openid,endtime,oname')
			->where($cond)
			->order('id desc')
			->page($page, $pagesize)
			->select();
