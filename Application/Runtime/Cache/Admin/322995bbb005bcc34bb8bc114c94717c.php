...
<aside class="right-side">
    ...
    <section class="content-header">
        <h1>
            地区编辑
             <a href="<?php echo U('Area/list');?>" class="btn btn-default pull-right"><font color="black">地区设置</font></a>
        </h1>
    </section>
    ...
    <!-- <section class="content"> -->
     ...
     {  
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box">
                     <form action="<?php echo U('Admin/area/edit',array('id'=>$row['id']));?>" method="post"  onsubmit="return submits();">
                    <select class="xiala" style= "margin-top:10px; margin-left:10px; margin-bottom:10px;" name="sid">
                        <?php if(is_array($cats)): foreach($cats as $key=>$c): if($row['sid'] == $c['id']): ?><option value ="<?php echo ($c['id']); ?>" selected><?php echo (str_repeat("&nbsp",$c['lev'])); echo ($c['mingcheng']); ?></option>
                        <?php else: ?>
                         <option value ="<?php echo ($c['id']); ?>"><?php echo (str_repeat("&nbsp",$c['lev'])); echo ($c['mingcheng']); ?></option><?php endif; endforeach; endif; ?>
        
                      </select>
                 <!-- <select class="xiala" name="mid">
                      <option value="1"><?php echo ($row['mid']); ?></option>
                         <option value="1">1</option>
                    
                      </select> -->
                <input type="text" name="mingcheng" class="text" value="<?php echo ($row['mingcheng']); ?>" placeholder="参数名称"/>

                <input type="submit"  class="btn btn-primary"  id ="submit" style="width:120px;" value="保存" class="sub_090" style="width:80px;"/>
                 </form>
