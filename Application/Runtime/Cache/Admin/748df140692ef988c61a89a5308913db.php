...
<aside class="right-side">
   ...
   <form id="user_search" action='<?php echo U('User/fetch4');?>' method="get" onsubmit="return checks();">
    ...
    <script type="text/javascript">
                                    ...
                                    //弹出图片src="/photo/avatar/m_<?php echo ((isset($user['avatar']) && ($user['avatar'] !== ""))?($user['avatar']):"avatar.png"); ?>" 
                                   var arr_img = [
                                        <?php if(is_array($user_list)): $i = 0; $__LIST__ = $user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?>'/photo/idpic/<?php echo ((isset($user['idpic']) && ($user['idpic'] !== ""))?($user['idpic']):"avatar.png"); ?>',<?php endforeach; endif; else: echo "" ;endif; ?>
                                    ];
